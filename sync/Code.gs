 ...
 *   2. Project Settings → Script properties → add SYNC_KEY = a long random
 *      passphrase. Only requests that send this key are answered.
 *   3. Deploy → New deployment → Web app. Execute as: Me. Who has access:
 *      Anyone. Copy the /exec URL.
 *   4. In HanziHome: Settings → Sync with Google Sheets → paste URL and key.
 *
 ...
 * cannot answer). The key travels in that body, never in the URL.
 *
 *   {action: "pull", key}                 -> {ok, rev, updatedAt, data}
 *   {action: "push", key, baseRev, data}  -> {ok, rev, updatedAt}
...
function doGet() {
  ...
  return json_({ ok: true, app: 'HanziHome sync', hint: 'POST {action, key} to use it.' });
}
...
function doPost(e) {
  ...
  if (!expected) return json_({ ok: false, error: 'not-configured' });
  if (!req.key || !sameString_(String(req.key), expected)) {
